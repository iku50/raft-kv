...
func TestInit(t *testing.T) {
	servers := 3
	chs := make([]chan raft.ApplyMsg, servers)
	peers := make([]proto.RaftClient, servers)
	persisters := make([]*raft.Persister, servers)
	ports := []string{
		"127.0.0.1:9991",
		"127.0.0.1:9992",
		"127.0.0.1:9993",
	...
	for i := range peers {
		peers[i] = facade.NewClient(ports[i])
		persisters[i] = raft.MakePersister()
	}
	for i := 0; i < servers; i++ {
		// chs[i] = make(chan ApplyMsg, 100)
		p := make([]proto.RaftClient, 0)
		for j := 0; j < servers; j++ {
			if j != i {
				p = append(p, peers[j])
			}
		}
		rf := raft.Make(p, int32(i), persisters[i], chs[i])
		hosts := strings.Split(ports[i], ":")
		go func(i int) {
			rpc.Start(hosts[1], rf)
		}(i)
	}
