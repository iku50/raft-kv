...
// 	logs        []map[int64]interface{} // copy of each server's committed entries
// 	lastApplied []int64
...
// 	cfg.logs = make([]map[int64]interface{}, cfg.n)
// 	cfg.lastApplied = make([]int64, cfg.n)
// 	cfg.start = time.Now()
//
...
// 	for i := 0; i < cfg.n; i++ {
// 		cfg.logs[i] = map[int64]interface{}{}
...
// func (cfg *config) ingestSnap(i int, snapshot []byte, index int64) string {
// 	if snapshot == nil {
...
// 				var xlog []interface{}
// 				for j := 0; int64(j) <= m.CommandIndex; j++ {
// 					xlog = append(xlog, cfg.logs[i][int64(j)])
// 				}
// 				e.Encode(xlog)
...
// 		cfg.endnames[i][j] = fmt.Sprintf("127.0.0.1:%v", 5643+i*100+j)
// 	}
//
// 	// a fresh set of ClientEnds.
// 	ends := make([]proto.RaftClient, cfg.n)
// 	for j := 0; j < cfg.n; j++ {
...
//
// 	rf := Make(ends, int32(i), cfg.saved[i], applyCh)
//
// 	cfg.mu.Lock()
// 	cfg.rafts[i] = rf
// 	cfg.mu.Unlock()
...
// func (cfg *config) checkTerms() int32 {
// 	term := int32(-1)
// 	for i := 0; i < cfg.n; i++ {
// 		if cfg.connected[i] {
...
// func (cfg *config) nCommitted(index int64) (int, interface{}) {
// 	count := 0
...
// func (cfg *config) wait(index int64, n int, startTerm int32) interface{} {
...
// func (cfg *config) one(cmd Command, expectedServers int, retry bool) int64 {
// 	t0 := time.Now()
// 	starts := 0
